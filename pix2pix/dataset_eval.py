...
class Pix2PixEvaluationDataset(Dataset):
    """
    ...
    Normalisation assumes the image is greyscale and has only one channel.
    ...
    def pad_image(self, image_arr, target_width, target_height):
        ...
        unchanged inside the padding. The image is a composition of the
    ...
    def __getitem__(self, idx):
        ...
        # save path to audio
        audio_path = os.path.join(config.RAW_DATA_ROOT, year,
                                  config.TARGET_MIC_NAME, loc, audio_key + '.wav')
        # save path to params
